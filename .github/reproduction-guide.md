...
3. Run `corepack pnpm install` to install dependencies
...
6. Run `corepack pnpm dev _community` to start the admin panel
...
- `payload-types.ts` - Generated types from `config.ts`. Generate this file by running `corepack pnpm dev:generate-types _community`.
...
- **Manually** - you can run all int tests in the `/test/_community/int.spec.ts` file by running the following command:
  ...
  ```bash
  corepack pnpm test:int _community
  ```
